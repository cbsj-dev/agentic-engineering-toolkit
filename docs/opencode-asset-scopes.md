...
| `git-publish` | Command | Creates or safely publishes a non-protected branch, stages reviewed changes, commits an approved description, and pushes it. | Run `/git-publish branch: feature/docs; files: docs/a.md, docs/b.md; message: auto`, or `/git-publish branch: current; files: all; message: docs: clarify setup`; the remote defaults to `origin`. |
